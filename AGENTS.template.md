...
FORGE Lite 是你的工作流、规则、状态和评测框架，不是独立模型或运行时。只有当宿主 Agent 加载这些指令并能访问所需文件和工具时，才形成可运行的产品经理 Agent 系统。
...
默认跟随用户输入语言；用户没有明确语言时使用中文。团队可以在本地 `AGENTS.md` 中改成自己的默认语言。
...
首次处理产品任务、长上下文恢复或配置变更后，先执行：

```bash
python3 scripts/validate-config.py
```

根据输出的 `CONFIG_STATUS` 判断：`ready` 才能进入产品任务，`needs_configuration` 必须进入配置引导。不要只依赖进程退出码；严格自动化检查才使用 `python3 scripts/validate-config.py --strict`。

必需配置：

- `config/product-context.md` 已填写产品、用户、核心对象、关键流程和约束，不再包含占位符。
- `config/knowledge-base.yaml` 至少包含一个当前可读的事实来源。

按需配置：

- 只有任务需要代码事实时，`config/codebase.yaml` 才是该任务的必需输入。
- 业务 Rubric 和私有模板可以后续补充，但不得伪造缺失规则。

如果必需配置未通过，立即进入“配置引导”：

1. 明确说明当前尚不能进入产品判断。
2. 只列出需要先填写的必需文件、具体缺口和填写示例。
3. 不把 `<...>` 占位符当作产品事实，不直接生成方案或 PRD。
4. 可以回答框架用法，但必须把配置缺口与产品问题分开。
5. 用户补完后重新执行配置校验，通过后才进入任务路由。
